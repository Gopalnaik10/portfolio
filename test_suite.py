...
test_email = os.getenv('TEST_ADMIN_EMAIL') or Config.ADMIN_EMAIL
test_password = os.getenv('TEST_ADMIN_PASSWORD') or Config.ADMIN_DEFAULT_PASSWORD

admin_authenticated = False
if not test_email or not test_password:
    print("[SKIP] Test 6: Admin test credentials not configured (set TEST_ADMIN_EMAIL / TEST_ADMIN_PASSWORD in .env). Skipping authenticated tests.")
else:
    ...
    try:
        ...
        if e.code == 401:
            print("[SKIP] Test 6: Admin login credentials did not authenticate (password may have been changed in dashboard). Set TEST_ADMIN_PASSWORD in .env to run authenticated tests.")
        else:
            raise
...
if admin_authenticated:
    ...
    print("[INFO] Skipped authenticated admin tests (Tests 7-12).")
    print("\n=== PUBLIC & UNAUTHENTICATED TESTS PASSED (1-5) ===")
